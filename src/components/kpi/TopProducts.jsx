...
function TrendBadge({ current, prev }) {
  if (!prev || prev === 0) {
    return <span className="text-xs italic" style={{ color: '#9b9b94' }}>New</span>
  }
  const pct = ((current - prev) / prev) * 100
  const up  = pct > 0
  const flat = Math.abs(pct) < 1
  if (flat) return (
    <span className="text-xs flex items-center gap-0.5" style={{ color: '#9b9b94' }}>
      <Minus size={11} /> 0%
    </span>
  )
  return (
...
function SortHeader({ label, sortKey, current, direction, onSort }) {
  ...
  return (
    <button
      ...
      style={{ color: active ? '#1a1a18' : '#9b9b94' }}
    >
      {label}
      <ArrowUpDown size={11} style={{ color: active ? '#1a1a18' : '#9b9b94' }} />
    </button>
  )
}
    ...
    >
      <div className="flex items-center justify-between" style={{ padding: '14px 16px', borderBottom: '1px solid rgba(0,0,0,0.08)' }}>
        <div>
          ...
          <div className="text-xs mt-0.5" style={{ color: '#9b9b94' }}>Ranked by revenue — click headers to sort</div>
        </div>
        <div className="text-xs" style={{ color: '#9b9b94' }}>
          FY Total: <span className="font-semibold" style={{ color: '#9b9b94', fontFamily: "'DM Mono', monospace" }}>${totalRevenue.toLocaleString()}</span>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr style={{ borderBottom: '1px solid rgba(0,0,0,0.08)', background: '#f0efe9' }}>
              <th className="text-left px-4 py-3 w-6 text-xs font-medium" style={{ color: '#9b9b94' }}>#</th>
              <th className="text-left px-3 py-3">
                <span className="text-xs font-medium uppercase tracking-wide" style={{ color: '#9b9b94' }}>Product</span>
              </th>
              ...
              <th className="text-right px-3 py-3">
                <span className="text-xs font-medium uppercase tracking-wide" style={{ color: '#9b9b94' }}>Trend</span>
              </th>
              <th className="text-right px-3 py-3 hidden lg:table-cell">
                <span className="text-xs font-medium uppercase tracking-wide" style={{ color: '#9b9b94' }}>Stock</span>
              </th>
            </tr>
          </thead>
          <tbody>
            {sorted.map((p, i) => {
              ...
              return (
                ...
                >
                  <td className="px-4 py-3 text-xs font-mono" style={{ color: '#9b9b94' }}>{i + 1}</td>
                  <td className="px-3 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      {lowStock && (
                        <span
                          ...
                          style={{
                            background: '#f0efe9',
                            color: criticalStock ? '#dc2626' : '#1a1a18',
                          }}
                        >
                      ...
                      {p.linkedProject && (
                        <button
                          ...
                          style={{ color: '#1a1a18', opacity: 0.6 }}
                    ...
                    <div className="text-xs mt-0.5" style={{ color: '#9b9b94' }}>{p.sku}</div>
                  </td>
                  <td className="px-3 py-3 text-sm font-medium tabular-nums" style={{ color: '#1a1a18', fontFamily: "'DM Mono', monospace", textAlign: 'right' }}>
                  ...
                  <td className="px-3 py-3 text-right hidden md:table-cell">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <span className="text-xs w-8 tabular-nums" style={{ color: '#9b9b94', fontFamily: "'DM Mono', monospace", textAlign: 'right' }}>
                  ...
                  <td className="px-3 py-3 text-right hidden lg:table-cell">
                    <span
                      ...
                      style={{
                        color: criticalStock ? '#dc2626' : lowStock ? '#1a1a18' : '#9b9b94',
